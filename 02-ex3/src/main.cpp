...
int main () {
  ...
  std::string fragmentShaderOrangeStr;
  std::string fragmentShaderYellowStr;
  ...
  fragmentShaderOrangeSource = fragmentShaderOrangeStr.c_str();
  fragmentShaderOrange = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(fragmentShaderOrange, 1, &fragmentShaderOrangeSource, NULL);
  glCompileShader(fragmentShaderOrange);
  glGetShaderiv(fragmentShaderOrange, GL_COMPILE_STATUS, &success);
  if (!success) {
    glGetShaderInfoLog(fragmentShaderOrange, 512, NULL, infoLogs);
    std::cerr << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLogs << std::endl;
  }
  
  /* fragm-s-02.glsl (yellow) */
  fragmentShaderYellowStr = loadShaderSource("build/fragm-s-02.glsl");
  fragmentShaderYellowSource = fragmentShaderYellowStr.c_str();
  fragmentShaderYellow = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(fragmentShaderYellow, 1, &fragmentShaderYellowSource, NULL);
  glCompileShader(fragmentShaderYellow);
  glGetShaderiv(fragmentShaderYellow, GL_COMPILE_STATUS, &success);
  if (!success) {
    glGetShaderInfoLog(fragmentShaderYellow, 512, NULL, infoLogs);
    std::cerr << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLogs << std::endl;
  }
...
/* first approach of setting up two programs, each with differente fragment shader */
  // /* fragment shader placed within an array*/
